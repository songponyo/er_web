    ...
    Swal.fire({
      ...
      if (result.value) {
        this.setState(
          ...
          async () => {
            user_position_model
              .deleteUserPositionByCode({ user_position_code: code })
              .then((res) => {
                if (res.require) {
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Card>
                  ...
                  render: (cell) => {
                    ...
                    if (permission_edit) {
                      row_accessible.push(
                        <Link
                          key="update"
                          to={`/user-position/update/${cell.user_position_code}`}
                          title="แก้ไขรายการ"
                        >
                          <button
                            type="button"
                    ...
                    if (permission_delete) {
                      row_accessible.push(
                        <button
                          ...
                          onClick={() =>
                            this._onDelete(cell.user_position_code)
                          }
                          title="ลบรายการ"
                        >
          ...
          <CardFooter>
            <Link to="/user">
              <Button color="btn btn-default">Back</Button>
            </Link>
          </CardFooter>
        </Card>
      </div>
