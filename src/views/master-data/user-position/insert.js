...
class Insert extends React.Component {
  ...
  async componentDidMount() {
    ...
    this.setState({
      showloading: false,
      code_validate: { value: max_code.data, status: 'VALID', class: '', text: '', },
    ...
    if (code.length) {
      if (this.state.code_validate.value !== code) {
        ...
        if (user_position.data.length) {
          this.setState({ code_validate: { value: code, status: 'INVALID', class: 'is-invalid', text: 'This code already exists.', } })
        } else {
          this.setState({ code_validate: { value: code, status: 'VALID', class: 'is-valid', text: '', } })
        }
      }
    } else {
          ...
          <Form onSubmit={this._handleSubmit.bind(this)}>
            ...
            <CardBody>
              <Row>
                <Col md="4">
                  <FormGroup>
                    ...
                    <Input
                      ...
                      onChange={(e) => this.setState({ user_code: e.target.value })}
                ...
                <Col md="4">
                  <FormGroup>
                    <Label>ชือตำแหน่ง <font color="#F00"><b>*</b></font></Label>
                    <Input
                      type="text"
                      ...
                      required
                    />
                    <p className="text-muted">Example : ฝ่ายดูแลระบบ.</p>
                  </FormGroup>
                </Col>
              </Row>
            </CardBody>
            <CardFooter>
              ...
              <Link to="/user-position"><Button type="button"> Back </Button></Link>
            </CardFooter>
          </Form>
        </Card>
      </div>
